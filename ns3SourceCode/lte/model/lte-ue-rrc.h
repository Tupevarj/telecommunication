...
private:
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //	CHANGE MADE HERE:
  // 		ADDED THREE TRACE SOURCES
  //		ADDED ONE ATTRIBUTE
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  TracedCallback<uint64_t, uint16_t, double> m_A2RsrqEnterTrace;
  ...
  TracedCallback<uint64_t, uint16_t, double, double> m_outOfSynchTrace;
  TracedCallback<uint64_t, uint16_t, double> m_radioLinkFailureTrace;


//  typedef void (* A3TracedCallback)
//     (uint64_t imsi, uint16_t cellId, double rsrp);
...
typedef void (*KpiTraceCallback)
   ...
   typedef void (* OutOfSynchCallback)
      (uint64_t imsi, uint16_t cellId, double rsrp, double thresh);


   typedef void (* RadioLinkFailureCallback)
      (uint64_t imsi, uint16_t cellId, double rsrp);

   typedef void (* EventEnterCallback)
		     (uint64_t imsi, uint16_t cellId, double rsr);

   typedef void (* EventLeaveCallback)
		     (uint64_t imsi, uint16_t cellIdl, double rsr);

   EventId m_outOfSynchTimeout;

   /**
    * The `Threshold value for low RSRP` attribute.
    */
   double m_rsrpThrLow;

   bool m_firstOutOfSynch = false;

   /**
    * The `T310` attribute. Timer used for to allow UE to get back in synchronization with
    * eNodeB
    */
   Time m_t310;
